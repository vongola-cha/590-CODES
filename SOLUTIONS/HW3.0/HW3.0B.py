...
#ANN PARAM
layers		=	[-1,5,5,1] #FIRST NUMBER OVERWRITTEN LATER BY INPUT SHAPE
...
if(model_type=="ANN"):
	...
	def NN_eval(X,submatrices):
		#print(X.shape)
		out=np.matmul(X,np.transpose(submatrices[0]))+np.transpose(submatrices[1]) 
		for i in range(2,int(len(submatrices)/2+1)):
			j=2*(i-1); #print(j)
			if(activation=="SIGMOID"): out=S(out) 
			if(activation=="TANH"):    out=2.0*(S(2.0*out)-0.5)
			out=np.matmul(out,np.transpose(submatrices[j]))+np.transpose(submatrices[j+1]) 
		return out 
